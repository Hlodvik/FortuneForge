    ...
    public async Task InitializeAsync()
    {
        ...
        if (!string.IsNullOrWhiteSpace(priorHost))
        {
            if (!priorHost.StartsWith("127.0.0.1:", StringComparison.OrdinalIgnoreCase) &&
                ...
                throw new InvalidOperationException("Blackjack table integration tests require a localhost Firestore emulator.");
            Endpoint = priorHost;
            return;
        }
        ...
        var configuration = Path.Combine(
            root, "FortuneForge.Server.Tests", "Blackjack", "Table", "firebase-emulator.test.json");
        emulatorWorkDirectory = Path.Combine(Path.GetTempPath(), $"fortuneforge-blackjack-table-{Guid.NewGuid():N}");
