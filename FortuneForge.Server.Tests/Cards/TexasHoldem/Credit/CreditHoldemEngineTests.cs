...
public sealed class CreditHoldemEngineTests
{
    ...
    [Fact]
    public void ProjectionPublishesExactShortAllInTarget()
    {
        var match = Deal(2);
        var viewer = match.Players[match.ActiveSeat];
        viewer.Stack = 5;
        viewer.CommittedRound = 0;
        viewer.CommittedHand = 0;
        viewer.Status = "active";
        viewer.HasActed = false;
        viewer.CanRaise = true;
        match.CurrentBet = 0;
        match.MinimumRaise = 10;

        var table = CreditHoldemProjection.Table(match, viewer.ActorId, Start);

        Assert.Contains(CreditHoldemActions.Raise, table.LegalActions);
        Assert.Equal(10, table.MinimumRaiseTo);
        Assert.Equal(5, table.MaximumRaiseTo);
        Assert.Equal(5, table.ShortAllInRaiseTo);
        var json = System.Text.Json.JsonSerializer.Serialize(
            table,
            new System.Text.Json.JsonSerializerOptions(System.Text.Json.JsonSerializerDefaults.Web));
        Assert.Contains("\"shortAllInRaiseTo\":5", json, StringComparison.Ordinal);
    }
