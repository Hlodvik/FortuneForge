...
Copy this directory to `games/<themeName>` and rename `manifest.template.ts.txt` to `manifest.ts`. A game is assembled from typed, replaceable sets rather than by copying the shared reel controller or Wukong.
...
Register the finished manifest once in `games/index.ts`. The catalog, routing, page title, authenticated route, and balance-free demo route are then derived from it.
