...
export function HeartsGame({ gateway, backHref = '/', playerName = 'Player', tableLabel = 'Local bot table' }: HeartsGameProps) {
  ...
  return <div className="ff-hearts-page" onClickCapture={onCardAudioClick}>
    ...
    <main className="ff-hearts-main">
      <section className="ff-hearts-title"><div><small>Take the fewest points to win</small><h1>Hearts</h1></div><div className="ff-hearts-start"><label htmlFor="target-score">Score limit</label><select id="target-score" value={targetScore} onChange={event => setTargetScore(Number(event.target.value))} disabled={busy} aria-label="Score limit; the player with the lowest score wins when someone reaches this total"><option value={50}>50</option><option value={100}>100</option><option value={150}>150</option></select><label htmlFor="bot-difficulty">Bots</label><select id="bot-difficulty" value={difficulty} onChange={event => setDifficulty(event.target.value as HeartsDifficulty)} disabled={busy} aria-label="Bot difficulty"><option value="relaxed">Relaxed</option><option value="standard">Standard</option><option value="sharp">Sharp</option></select><button type="button" data-card-audio="shuffle" onClick={startMatch} disabled={!serviceReady || busy}>{busy ? 'Dealing…' : 'New game'}</button></div></section>
      {match ? <>
