import { HeartsGatewayError, type HeartsGateway, type HeartsMatch, type HeartsSeat, type HeartsStatus } from './contracts'
...
export class HttpHeartsGateway implements HeartsGateway {
  ...
  startMatch(options: { targetScore: number; seed?: number }, signal?: AbortSignal) { return this.request('/matches', { method: 'POST', headers: jsonHeaders, body: JSON.stringify(options), signal }, isMatch) }
