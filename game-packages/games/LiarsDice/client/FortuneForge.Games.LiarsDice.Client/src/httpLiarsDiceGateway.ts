...
export class HttpLiarsDiceGateway implements LiarsDiceGateway {
  ...
  spotOn(matchId: string, signal?: AbortSignal) { return this.request(`/matches/${encodeURIComponent(matchId)}/spot-on`, { method: 'POST', signal }, isMatch) }
...
function isOutcome(value: unknown): boolean { return isRecord(value) && typeof value.challengerId === 'string' && typeof value.bidderId === 'string' && typeof value.loserId === 'string' && Number.isInteger(value.quantity) && Number.isInteger(value.face) && Number.isInteger(value.matchingDice) && (value.callType === undefined || value.callType === 'liar' || value.callType === 'spot-on') }
