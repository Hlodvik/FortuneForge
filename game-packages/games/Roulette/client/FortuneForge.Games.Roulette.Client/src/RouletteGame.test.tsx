...
describe('RouletteGame', () => { it('renders a playable single-zero table shell', () => { const markup = renderToStaticMarkup(createElement(RouletteGame, { gateway })); expect(markup).toContain('Roulette'); expect(markup).toContain('Single-zero table'); expect(markup).toContain('Other games'); expect(markup).toContain('0'); expect(markup).toContain('Red'); expect(markup).toContain('Tips'); expect(markup).toContain('Place bets, then spin') }) })
