  ...
  return <div className="ff-2048-page">
    ...
    <main className="ff-2048-main">
      ...
      {game ? <div className="ff-2048-gameplay">
        <section className="ff-2048-stats" aria-live="polite"><div><small>Score</small><strong>{game.score.toLocaleString()}</strong></div><div><small>Best tile</small><strong>{game.highestTile.toLocaleString()}</strong></div><div><small>Moves</small><strong>{game.moves}</strong></div></section>
        <section className="ff-2048-board-wrap" aria-label="2048 board"><div className="ff-2048-board" style={{ '--board-size': game.size } as CSSProperties}>{Array.from({ length: game.size * game.size }, (_, index) => <div className="ff-2048-cell" key={index} />)}<div className="ff-2048-tile-layer">{game.tiles.map((value, index) => value ? <div className={`ff-2048-tile ${tileClass(value)}${revealingTiles.includes(index) ? ' ff-2048-tile--revealing' : ''}`} key={index} style={tilePosition(index, game.size)} aria-label={`Tile ${value}`}>{tileLabel(value)}</div> : null)}{tileMotion.map((tile, index) => <div className={`ff-2048-tile ${tileClass(tile.value)} ff-2048-tile--moving`} key={`${tile.from}-${tile.to}-${index}`} style={motionPosition(tile, game.size)} aria-hidden="true">{tileLabel(tile.value)}</div>)}</div></div>{game.phase !== 'playing' && <div className="ff-2048-overlay"><small>{game.phase === 'won' ? 'You made it' : 'Board locked'}</small><strong>{game.phase === 'won' ? '2048 reached!' : 'No moves remain'}</strong><span>{game.phase === 'won' ? 'That is a winning game.' : 'Try a new board and find a different path.'}</span><div><button type="button" onClick={newGame} disabled={busy}>New game</button>{game.canUndo && <button type="button" onClick={undo} disabled={busy}>Undo</button>}</div></div>}</section>
        ...
        <p className="ff-2048-help">Use your keyboard arrows or the controls. Every legal move adds one new tile.</p>
